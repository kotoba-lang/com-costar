(ns costar.main
  "Kotodama WASM entrypoint for the Costar clean-room actor (L4) — Clojure port.
  ...
  facts (`costar.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "costar")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"costar.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "costar-compat" :tier tier :entities entities} 200])
...
(defn start! [] :costar-compat/ready)
